...
namespace strategy {
RobotMarker::RobotMarker(u_int8_t robot_id) { this->target_robot_id = robot_id; }

std::optional<RobotIntent> RobotMarker::get_task(RobotIntent intent, const WorldState* world_state,
    ...
    auto target_position =
        (world_state->get_robot(false, target_id).pose.position() - ball_position) * factor +
        ball_position;
